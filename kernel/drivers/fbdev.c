...
void fbdev_init_device(struct limine_framebuffer *framebuffer) {
	...
	*device->var = (struct fb_var_screeninfo) {
		...
		.grayscale = 0,
		.red = (struct fb_bitfield) { framebuffer->red_mask_shift, framebuffer->red_mask_length, 0 },
		.green = (struct fb_bitfield) { framebuffer->green_mask_shift, framebuffer->green_mask_length, 0 },
		.blue = (struct fb_bitfield) { framebuffer->blue_mask_shift, framebuffer->blue_mask_length, 0 },
